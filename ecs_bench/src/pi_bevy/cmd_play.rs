...
use bevy_ecs::prelude::{Schedule, SystemStage, StageLabel, Stage};
use bevy_app::App;
use pi_hash::XHashMap;
use pi_idtree1::IdTree;
use pi_ui_render_bevy::{prelude::UiPlugin, system::node::user_setting::user_setting, export::PlayContext, export::{*, json_parse::as_value}, resource::{UserCommands, ExtendCssCmd}};
...
impl Benchmark {
    pub fn new() -> Self {
		...
		app
			// .add_plugins(bevy_internal::DefaultPlugins)
			...
			.add_stage_before(CoreStage::First, InitStartupStage::Startup, SystemStage::parallel().with_run_criteria(ShouldRun::once))
			// .add_system_to_stage(InitStartupStage::Startup, move | commands: Commands| {
			// 	log::warn!("init=======================");
			// 	exmple1.lock().init(commands, (500, 500));
			// }).run()
		;
		...
		let mut commands = UserCommands::default();
        ...
        let mut cb = |dwcss: &DirEntry| {
            ...
            if let Ok(r) = file {
				create_class_by_bin(&mut commands, r.as_slice());
        ...
        let full_screen_class = pi_style::style_parse::parse_class_map_from_string(full_screen_class.as_str(), 0).unwrap();
		commands.push_cmd(ExtendCssCmd(vec![full_screen_class]));
        ...
        let root = play_create_node(&mut app, context, &vec![JsonValue::Object(json.clone())]);
        play_width(
            &mut commands,
            context,
		...
		while setting(&mut list_index, &mut json_arr, cmd_path, play_path, play_version, &mut file_index, &mut app, &mut commands, &mut play_context) {}
...
pub fn setting(list_index1: &mut usize, json_arr: &mut JsonValue, cmd_path: Option<&str>, play_path: &str, play_version: &str, file_index1: &mut usize, app: &mut App, commands: &mut UserCommands, play_context: &mut PlayContext) -> bool{
				...
				if let JsonValue::Object(r) = play_item {
					...
					if ret == 0.0 {
						if let JsonValue::Array(param) = param {
							if ty >= 57 && ty <= 61 {
								if let Some(cmd) = CREATE_CMD_LIST.get(ty) {
									cmd(app, play_context, param);
								}
							} else {
								if let Some(cmd) = CMD_LIST.get(ty) {
									cmd(commands, play_context, param);
								}
							}
						...
						if ty >= 57 && ty <= 61 {
							...
							if let Some(cmd) = CMD_LIST.get(ty) {
								cmd(commands, play_context, &vec![play_item.clone()]);
							}
						}
					}
				}
...
lazy_static::lazy_static! {
    pub static ref CMD_LIST: Vec<fn (&mut UserCommands, &mut PlayContext, &Vec<json::JsonValue>) > = vec![
        // 布局
...
pub fn play_todo(_gui: &mut UserCommands, _context: &mut PlayContext, _json: &Vec<json::JsonValue>) {}
pub fn play_todo_create(_gui: &mut App, _context: &mut PlayContext, _json: &Vec<json::JsonValue>) {}
...
pub fn set_atom(_gui: &mut UserCommands, context: &mut PlayContext, json: &Vec<json::JsonValue>) {
